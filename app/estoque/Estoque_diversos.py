...
"""
Caminhos onde serão salvos os daos coletados de estoque
Farei alteração no modo de extração futuramente
irá pegar um arquivo único e separar em pedaços no código
"""
ARQUIVOS = {
...
def _normalizar_xlsx(caminho_xlsx: Path) -> None:
    """
    O formato do arquivo que do qual é extraído o relatório tem uma codificação diferente
    Dá muito problma na hora de extrair com openpyxl

    Args:
        caminho_xlsx (object): arquivo a ser manipulado

        caminho_tmp (object): eu sempre priorizo trabalhar com um arquivo temporário
        então opto por criar uma cópia do arquivo.

        houve_ajuste (tuple): verifica se deu tudo certo com a normalização do arquivo
    """
